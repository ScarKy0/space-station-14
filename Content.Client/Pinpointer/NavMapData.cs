...
public sealed class NavMapData
{
    ...
    public Entity<MapGridComponent> Entity;

    // Default colors
    public Color WallColor = Color.ToSrgb(new Color(102, 217, 102));
    public Color TileColor = new Color(65, 252, 3);
    ...
    // TODO: Subscribe to statechanges on navmapcomponent
    ...
    public void Draw(DrawingHandleBase handle, Func<Vector2, Vector2> scale, Box2 localAABB)
    {
        ...
        {
            verts.Clear();
            var tilesEnumerator = maps.GetLocalTilesEnumerator(Entity, Entity, localAABB);

            while (tilesEnumerator.MoveNext(out var tileRef))
            {
                if (!TilePolygons.TryGetValue(tileRef.GridIndices, out var polys))
                    continue;

                for (var i = 0; i < polys.Length; i++)
                {
                    verts.Add(scale.Invoke(polys[i]));
                }
            }

            handle.DrawPrimitives(DrawPrimitiveTopology.TriangleList, verts.Span, TileColor);
        }
    ...
    public void UpdateNavMap(Entity<MapGridComponent?> entity)
    {
        if (!_entManager.TryGetComponent(entity.Owner, out entity.Comp))
            return;

        Entity = (entity.Owner, entity.Comp);
        ...
        UpdateNavMapFloorTiles(entity.Owner);
        UpdateNavMapWallLines((entity.Owner, entity.Comp, null));
        UpdateNavMapAirlocks((entity.Owner, entity.Comp, null));
    }
    ...
    private void UpdateNavMapFloorTiles(Entity<MapGridComponent?> entity)
    {
        ...
        {
            var box = lookup.GetLocalBounds(tile.Value.GridIndices, entity.Comp.TileSize).Enlarged(-0.45f);
            box = new Box2(box.Left, -box.Bottom, box.Right, -box.Top);
            var arr = new Vector2[6];

            arr[0] = box.BottomLeft;
            arr[1] = box.BottomRight;
            arr[2] = box.TopLeft;

            arr[3] = box.BottomRight;
            arr[4] = box.TopLeft;
            arr[5] = box.TopRight;
